...
export function getWidgetScript(projectId: string, primaryColor: string): string {
  return `
<div id="vd-badge" style="position:fixed;bottom:16px;right:16px;z-index:999;display:flex;align-items:center;gap:8px;background:#fff;border:1px solid #e5e7eb;border-radius:12px;padding:8px 14px;box-shadow:0 2px 12px rgba(0,0,0,.08);font-family:Inter,-apple-system,sans-serif;cursor:pointer;transition:all .2s;max-width:200px" onclick="window.open('https://bridgegap.app','_blank')" onmouseover="this.style.transform='translateY(-2px)';this.style.boxShadow='0 6px 20px rgba(0,0,0,.12)'" onmouseout="this.style.transform='';this.style.boxShadow='0 2px 12px rgba(0,0,0,.08)'">
  <span style="font-size:14px">✨</span>
  <div>
    <div style="font-size:11px;font-weight:600;color:#111827;line-height:1.3">Built with BridgeGap</div>
